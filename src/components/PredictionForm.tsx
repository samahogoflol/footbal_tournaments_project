...
export default function PredictionForm({
  ...
  const getStatusLabel = () => {
    if (matchStatus === 'live') return 'Матч вже триває';
    if (matchStatus === 'finished') return 'Матч завершено';
    if (isTooFarAway) return 'Прогнози ще не відкриті';
    if (isLocked) return 'Прийом прогнозів закрито';
    return 'Ваш прогноз';
  };

  const getStatusIcon = () => {
    if (isTooFarAway) return <Clock size={14} />;
    ...
    <div className="bg-zinc-900/50 rounded-3xl border border-zinc-800 p-6 mb-8">
      ...
      {isTooFarAway ? (
        <div className="text-center py-4 text-zinc-500 text-sm">
          Прогнози відкриються за 7 днів до матчу
        </div>
      ) : (
        <>
